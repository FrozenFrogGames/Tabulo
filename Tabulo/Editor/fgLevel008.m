...
- (void)buildSceneForLevel:(fgTabuloDirector *)_director withStrategy:(fgTabuloStrategy *)_strategy {
    ...
    fgHouseNode *node6 = [(fgTabuloStrategy *)_strategy buildHouseNode:[scene getPointAt:6] extend:CGSizeMake(0.8f, 0.8f) writer:dataWriter symbols:dataSymbols];
    ...
    [scene buildHouse:_director node:node6 type:TABULO_PawnOne state:_strategy writer:dataWriter symbols:dataSymbols];
    ...
    f3ViewAdaptee *plankOne = [scene buildSmallPlank:_director state:_strategy node:node7 angle:90.f hole:TABULO_HOLE_MAX writer:dataWriter symbols:dataSymbols];
    [scene buildDragPlankControl:_director strategy:_strategy node:node7 view:plankOne writer:dataWriter symbols:dataSymbols];
    
    f3ViewAdaptee *plankTwo = [scene buildMediumPlank:_director state:_strategy node:node1 angle:270.f hole:TABULO_HOLE_MAX writer:dataWriter symbols:dataSymbols];
    [scene buildDragPlankControl:_director strategy:_strategy node:node1 view:plankTwo writer:dataWriter symbols:dataSymbols];
